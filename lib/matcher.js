/**
 * 四维度匹配引擎（类别 + 标签 + 关键词 + 屏蔽词）
 *
 * 匹配模型（规则订阅制）：
 * ┌─────────────┐     ┌───────────────┐     ┌──────────┐     ┌──────────┐
 * │  类别(一级)  │  ×  │  标签(二级子分类)│  ×  │  关键词   │  ×  │  屏蔽词   │
 * │  OR 匹配     │     │  OR 匹配       │     │  OR 匹配  │     │  NOT 匹配 │
 * └─────────────┘     └───────────────┘     └──────────┘     └──────────┘
 *         │                  │                    │                │
 *         └─────────── AND ──┴─────── AND ────────┴────── AND ────┘
 *
 * 前三个维度正向匹配（命中 = 通过），屏蔽词反向过滤（命中 = 排除）。
 *
 * 防爆炸策略：
...
/**
 * 屏蔽词匹配（OR）— 反向过滤器
 * 命中任一屏蔽词即排除该帖（与正向维度相反：true = 应屏蔽）
 */
function matchBlockedWords(text, blockedWords) {
  if (!blockedWords || blockedWords.length === 0) return false; // 无屏蔽词 = 不屏蔽
  if (!text) return false;
  const lower = text.toLowerCase();
  return blockedWords.some(kw => lower.includes(kw.toLowerCase()));
}
...
export function matchPostAgainstRule(post, rule) {
  ...
  if (catOk && tagOk && kwOk) {
    // 维度 4：屏蔽词 — 命中任一屏蔽词即排除此帖
    if (matchBlockedWords(textToSearch, rule.blockedWords)) return null;
