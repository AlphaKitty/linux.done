...
// 持久缓存 TTL — 6 小时（跨页面刷新）
const PERSIST_CACHE_TTL = 21_600_000;

const CACHE_KEYS = {
  CATEGORIES: 'cache_categories_v2',
  TAGS: 'cache_tags_v2',
};
...
export async function getTags() {
  ...
  // 3) 网络请求 — 双端点合并，覆盖受限标签组中的标签
  //    /tags.json — 标准公开标签
  //    /tags/filter/search.json — 含受限标签（如按类别限制的标签组）
  let data, searchData;
  try {
    [data, searchData] = await Promise.all([
      fetchWithRetry(`${BASE_URL}/tags.json`),
      fetchWithRetry(`${BASE_URL}/tags/filter/search.json?q=`).catch(() => null),
    ]);
  } catch {
    // 网络失败时回退到持久缓存
  ...
  // 主端点标签
  const mainTags = (data?.tags || []).map(t => ({
    name: t.name,
  ...
  const searchTags = searchResults.map(t => ({
    name: t.name || t.text || '',
    topicCount: t.topic_count || t.count || 0,
  })).filter(t => t.name);

  // 合并去重 — 主端点优先（topicCount 更准确）
  const seen = new Set();
  const tags = [];
  for (const t of [...mainTags, ...searchTags]) {
    if (!seen.has(t.name)) {
      seen.add(t.name);
      tags.push(t);
    }
  ...
  // 清理旧版缓存键（一次性迁移）
  chrome.storage.local.remove(['cache_tags', 'cache_categories']).catch(() => {});
  return tags;
}
